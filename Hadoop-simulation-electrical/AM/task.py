...
class MapRJ:
	...
	def map(self, env, pipes2Reduce, pipe2Epath):
		...
		if self.size > 0 and self.inputRackId != self.cntrRackId:
			...
			print str(env.now) + ': finishing moving map input from a different rack'
		else:
...
# 				print str(self.numFinishMaps) + ': reduce task ' + str(self.taskId) + ' receives a message from map task ' + str(msg[1]) + ' at time ' + str(env.now)
				if msg[3] > 0:
# 					print str(env.now) + ': ' + self.jobId + ' shuffling from map ' + str(msg[1]) + ' on rack ' + str(msg[2]) + ' to reduce ' + str(self.taskId) + ' on rack ' + str(self.cntrRackId) + ' starts'
...
# 				print str(env.now) + ': ' + self.jobId + ' shuffling from map ' + str(msg[1]) + ' to reduce ' + str(self.taskId) + ' finishes'
				self.numFinishFlow += 1	
