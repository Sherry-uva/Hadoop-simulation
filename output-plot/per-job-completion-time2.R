...
jobInfo <- read.csv(file="/Users/sherry/Box Sync/Hadoop-simulation-hybrid/logs/duration1000sec_SHJRatio5%_arrivalRate0.05-cpuUtil1.csv", header=TRUE, sep=",")
...
jobInfo <- read.csv(file="/Users/sherry/Box Sync/Hadoop-simulation-hybrid/logs-0%/duration1000sec_SHJRatio5%_arrivalRate0.05-cpuUtil1.csv", header=TRUE, sep=",")
jobInfo <- as.matrix(jobInfo)

complTime_hybrid_0 <- jobInfo[,3]
cpu_hybrid_0 <- jobInfo[,4]

jobInfo <- read.csv(file="/Users/sherry/Box Sync/Hadoop-simulation-electrical/logs/duration1000sec_SHJRatio5%_arrivalRate0.05-cpuUtil1.csv", header=TRUE, sep=",")
jobInfo <- as.matrix(jobInfo)

complTime_electrical <- jobInfo[,3]
cpu_electrical <- jobInfo[,4]

plot(range(c(0,length(jobId))), range(c(0, 400)), axes=FALSE, type="n",xlab='job indices',ylab='job completion time (s)',cex.lab=1.2)
box()
axis(2, at=(0:4)*100,cex.axis=1.5)
axis(1, at=(0:11)*5,cex.axis=1)
