...
jobInfo <- read.csv(file="/Users/sherry/Box Sync/Hadoop-simulation-hybrid/logs/duration500sec_SHJRatio5%_arrivalRate0.2-cpuUtil1.csv", header=TRUE, sep=",")
...
jobInfo <- read.csv(file="/Users/sherry/Box Sync/Hadoop-simulation-hybrid/logs-0%/duration500sec_SHJRatio5%_arrivalRate0.2-cpuUtil1.csv", header=TRUE, sep=",")
jobInfo <- as.matrix(jobInfo)

complTime_hybrid_0 <- jobInfo[,3]
cpu_hybrid_0 <- jobInfo[,4]

jobInfo <- read.csv(file="/Users/sherry/Box Sync/Hadoop-simulation-electrical/logs/duration500sec_SHJRatio5%_arrivalRate0.2-cpuUtil1.csv", header=TRUE, sep=",")
jobInfo <- as.matrix(jobInfo)
...
plot(range(c(0,length(jobId))), range(c(0, 500)), axes=FALSE, type="n",xlab='job indices',ylab='job completion time (s)',cex.lab=1.2)
box()
axis(2, at=(0:5)*100,cex.axis=1.5)
axis(1, at=(0:8)*10,cex.axis=1)
