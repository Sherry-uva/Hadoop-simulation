...
MAPPING_CNTR_TO_HOST, MAPPING_CNTR_TO_RACK, MAPPING_HOST_TO_RACK, NUM_N0, NM_HEARTBEAT_INTERVAL, CPU_UTIL_UPDATE_MAX
import log as L
...
def allocateOneFreeCntr(jobId, cntrId, currTime):
	...
	# only if the container is in a rack connected to the OCS, the container will be allocated to a rack-queue request
	if rackId < NUM_N0: 
		# try to allocate the container to a rack-queue request
		for index in range(len(RM.rackQueues[rackId])): 
			request = RM.rackQueues[rackId][index]
			if len(request) == 2: # map-cntr request
				if request[1] > 1:	
					RM.rackQueues[rackId][index][1] -= 1
				else:
					del RM.rackQueues[rackId][index]
				RM.AMs[request[0]].appMaster_proc.interrupt(['containerAllocation', [['m', cntrId]]])
				return 
			else: # reduce-cntr request
				if request[2] == 'waiting':
					continue
				if request[1] > 1:	
					RM.rackQueues[rackId][index][1] -= 1
				else:
					del RM.rackQueues[rackId][index]
				RM.AMs[request[0]].appMaster_proc.interrupt(['containerAllocation', [['r', cntrId]]])
				return
	if len(RM.AMQueue) > 0:
...
# try to allocate idle containers in a rack to newly added map- or reduce-container request of a SHJ
def tryAlloCntrRackQ(rackId, jobId):
# 	print 'SHJ ' + jobId + ' requests containers on rack ' + str(rackId)
	global RM, cntrStatus, NUM_HOST_PER_RACK, NUM_CNTR_PER_HOST
# 	pdb.set_trace()
	cntrAllo = []
	tmp = 0
	while jobId not in RM.rackQueues[rackId][tmp]:
		tmp += 1
	numReqCntr = RM.rackQueues[rackId][tmp][1]
	if len(RM.rackQueues[rackId][tmp]) == 2:
		cntrType = 'm'
	else:
		cntrType = 'r'
	while numReqCntr > 0:
		cntrId = findIdleCntrInRack(rackId)
		if cntrId == -1:
			break
		else:
			numReqCntr -= 1	
			cntrAllo.append([cntrType, cntrId])		
	if numReqCntr != 0:
		RM.rackQueues[rackId][tmp][1] = numReqCntr
	else:
		del RM.rackQueues[rackId][tmp]
	if len(cntrAllo) > 0:
		RM.AMs[jobId].appMaster_proc.interrupt(['containerAllocation', cntrAllo])
...
def findIdleCntrInRack(rackId):
	global cntrStatus, NUM_HOST_PER_RACK, NUM_CNTR_PER_HOST
	setHosts = range(rackId*NUM_HOST_PER_RACK, (rackId+1)*NUM_HOST_PER_RACK)
	while setHosts != []:
		host = random.choice(setHosts)
		setHosts.remove(host)
		if 0 in cntrStatus[host]:
			localIndex = cntrStatus[host].index(0)
			cntrStatus[host][localIndex] = 1
			return NUM_CNTR_PER_HOST*host + localIndex
	# cannot find an idle container
	return -1	
